...
ndch1<-read.table(file = "diversity_neutrality_grouped_3_pops/A.aegypti_TajimasD_chrom1_12pops_3Groups_50k_5K.txt", header = TRUE)
ndch1$chrom<-rep("1",nrow(ndch1))

ndch2<-read.table(file = "diversity_neutrality_grouped_3_pops/A.aegypti_TajimasD_chrom2_12pops_3Groups_50k_5K.txt", header = TRUE)
ndch2$chrom<-rep("2",nrow(ndch2))

ndch3<-read.table(file = "diversity_neutrality_grouped_3_pops/A.aegypti_TajimasD_chrom3_12pops_3Groups_50k_5K.txt", header = TRUE)
...
### quantile (pi)
the_quantile <- 0.05
quantile_td_G1 <- quantile (data_pre[,"A.ae1_TD"], the_quantile,na.rm = T)
quantile_td_G2 <- quantile (data_pre[,"A.ae2_TD"], the_quantile,na.rm = T)
quantile_td_G3 <- quantile (data_pre[,"A.ae3_TD"], the_quantile,na.rm = T)


the_quantile <- 0.95
quantile_td2_G1 <- quantile (data_pre[,"A.ae1_TD"], the_quantile,na.rm = T)
quantile_td2_G2 <- quantile (data_pre[,"A.ae2_TD"], the_quantile,na.rm = T)
quantile_td2_G3 <- quantile (data_pre[,"A.ae3_TD"], the_quantile,na.rm = T)


pdf(file = "~/Documents/malaria_vector_reserach/input_data/a.aegypti/baypass/diversity_neutrality_grouped_3_pops/A.aegypti_grouped3_pops_TD_D.pdf", width = 8, height = 4)
for (i in 1:length(g_coor_gene)){
  ...
  region<-aa[aa$start>(genes_top10_focal$start)-1100000 & aa$stop<(genes_top10_focal$stop)+1100000,]
  #region$mid_good<-region$mid/1000000
  
  
  plot(region$mid,region$A.ae1_TD , type = "l",lwd = 1.8, col = "darkcyan", xlab = "Position (Mb)",las = 1,ylab="Tajima's D",  ylim=c(-2,6), main=paste("A.aegypti_",g_coor_gene[i]))
  mtext(side=2, text="pi", line=4.5)
  
  lines(region$mid, region$A.ae2_TD, type="l",lwd = 1.5, col = "dodgerblue4")
  lines(region$mid, region$A.ae3_TD, type="l",lwd = 1.5, col = "sienna3")
  ...
  abline(h =c(quantile_td_G1,quantile_td_G2,quantile_td_G3) ,col=c("darkcyan","dodgerblue4","sienna3"), lwd=0.7, lty=4)
  abline(h =c(quantile_td2_G1,quantile_td2_G2,quantile_td2_G3) ,col=c("darkcyan","dodgerblue4","sienna3"), lwd=0.7, lty=4)
...
pdf(file = "A.aegypti_grouped3_pops_TD_Boxplots_BayPass_MVMT_top_genes_TRUE.pdf", width = 7, height = 5)
for (i in 1:length(g_coor_gene)){
  ...
  ## G1
  region_G1<-region[,c("start","stop","mid","A.ae1_TD","chrom")]
  region_G1$group<-rep("A.ae1",nrow(region_G1))
  region_G1$status<-rep("focal",nrow(region_G1))
  
  nonregion_G1<-nonregion[,c("start","stop","mid","A.ae1_TD","chrom")]
  nonregion_G1$group<-rep("A.ae1",nrow(nonregion_G1))
  nonregion_G1$status<-rep("wholegenome",nrow(nonregion_G1))
  G1<-do.call("rbind", list(region_G1,nonregion_G1))
  colnames(G1)<-c("start","stop","mid", "A.ae_TD","chrom","group","status")
  
  ## G2
  region_G2<-region[,c("start","stop","mid","A.ae2_TD","chrom")]
  region_G2$group<-rep("A.ae2",nrow(region_G2))
  region_G2$status<-rep("focal",nrow(region_G2))
  
  nonregion_G2<-nonregion[,c("start","stop","mid","A.ae2_TD","chrom")]
  nonregion_G2$group<-rep("A.ae2",nrow(nonregion_G2))
  nonregion_G2$status<-rep("wholegenome",nrow(nonregion_G2))
  G2<-do.call("rbind", list(region_G2,nonregion_G2))
  colnames(G2)<-c("start","stop","mid", "A.ae_TD","chrom","group","status")
  
  
  ## G3
  region_G3<-region[,c("start","stop","mid","A.ae3_TD","chrom")]
  region_G3$group<-rep("A.ae3",nrow(region_G3))
  region_G3$status<-rep("focal",nrow(region_G3))
  
  nonregion_G3<-nonregion[,c("start","stop","mid","A.ae3_TD","chrom")]
  nonregion_G3$group<-rep("A.ae3",nrow(nonregion_G3))
  nonregion_G3$status<-rep("wholegenome",nrow(nonregion_G3))
  G3<-do.call("rbind", list(region_G3,nonregion_G3))
  colnames(G3)<-c("start","stop","mid", "A.ae_TD","chrom","group","status")
  ...
  lvl2=c("A.aeG1","A.aeG2","A.aeG3")
  boxplot(A.ae_TD~group*status, data=allGroup,notch=TRUE,at=at_1,xaxt="n",cex.axis=1.5,las = 3,frame.plot = FALSE,xlab="",ylab="",cex.lab=2.5,labs=2,col=c(cols[3],"grey20",cols[2],"grey50",cols[1],"grey80"),outline=FALSE,ylim=c(-2,5), main =paste("A.aegypti_",g_coor_gene[i]) )
  axis(1,at=at,labels=lvl2,tick=T,cex.axis=1.5)
  mtext(expression(TD),las=1,side=2,line=4,cex=3)
